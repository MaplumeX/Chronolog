...
Layout: desktop expand/collapse the sidebar (icon rail reclaims width). At a narrow window (`<768px`) verify the bottom tab bar (timer/stats/goals/categories/tags) switches pages and the top-right settings icon reaches the settings page — the sidebar drawer no longer exists on mobile (task 09-08-mobile-responsive). Also check touch-specific behavior if available: week-view horizontal swipe, row actions always visible (no hover), gap-slot tap creating an entry. Editing flows are bottom sheets on mobile (task 09-09-mobile-editing-sheet): block tap / gap ghost card / entries-view card open the entry editor as a sheet (delete link + merge row + cancel/save row); categories/tags ⋯ menu → edit / add sub-item open their forms as sheets; inside a sheet, the CategoryPicker/TagPicker dropdowns and the nested ConfirmDialog/MergeDialog must still open (modal-in-modal — jsdom cannot catch breakage here, real-browser check required).
