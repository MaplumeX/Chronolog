...
Footer: user entry only (shows `displayName ?? username`, clickable → settings page). Language switcher, theme switcher, logout, and API tokens moved into the tabbed settings page — do not re-add them to the sidebar. While a timer runs, the 计时 item shows elapsed (`formatDuration`) via `SidebarMenuBadge`. Do not add Calendar, Timesheet, week pickers, or a marketing landing page.
