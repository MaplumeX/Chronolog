...
- 分类/标签颜色可落库（`color` = 1–8 色板索引，任务 08-30-category-tag-color-palette）：展示位用 `paletteColor(color, fallbackName)` / `paletteForegroundColor(color, fallbackName)`（format.ts）——显式色优先返回 `var(--category-N)`，NULL 回退 hash 色。**`categoryIndex`/`categoryColor` hash 逻辑不可改动**，否则既有回退映射漂移。色板 UI 复用 `web/src/components/ColorPalettePicker.tsx`（8 色点 + 自动，radiogroup），编辑浮窗复用 `NameColorEditPopover.tsx`。
