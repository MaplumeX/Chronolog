...
- 分类/标签颜色可落库（`color` = 1–8 色板索引，任务 08-30-category-tag-color-palette）：展示位用 `paletteColor(color, fallbackName)` / `paletteForegroundColor(color, fallbackName)`（format.ts）——显式色优先返回 `var(--category-N)`，NULL 回退名称 hash 色（仅防御性回退——存量 NULL 已由服务端启动迁移固化，task 09-09）。**创建即固定**（任务 08-30-palette-auto-to-fixed）：新建时前端用 trim 后名称 hash（`categoryIndex(name)+1`）作为 color 随 POST 落库，编辑色板 8 色点必选其一（无「自动」选项）；旧 NULL 数据编辑时默认选中 hash 回退色、保存固化。`categoryIndex` 为 FNV-1a 32-bit hash（offset basis 0x811c9dc5、prime 0x01000193、`Math.imul` 乘法、对 8 取模，task 09-09 起）；服务端 `server/src/color-hash.ts` 为逐字一致的双实现（单仓库无共享包机制，两侧测试 `web/src/format.test.ts` ↔ `server/test/color-hash.test.ts` 用同一批已知向量锚定，改动必须同步两侧）。服务端 `migrate()`（`server/src/db.ts`）启动时把 `categories` / `tags` 中 `color IS NULL` 的行按新 hash 固化回填 1–8（幂等，只动 NULL 行），注册默认分类也直接落色，不留 NULL。色板 UI 复用 `web/src/components/ColorPalettePicker.tsx`（8 色点，radiogroup），编辑浮窗复用 `NameColorEditPopover.tsx`。
