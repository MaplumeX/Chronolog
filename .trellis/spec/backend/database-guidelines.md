...
| Table | Notes |
|-------|--------|
| `users` | `username` unique with `COLLATE NOCASE`; `password_hash` Argon2id PHC; `display_name` nullable (added task 08-29-user-system); `timezone` TEXT nullable — NULL = 未设置，前端回退浏览器时区（added task 09-06-settings-timezone）; `continuous_timing` INTEGER NOT NULL DEFAULT 0 — SQLite boolean, 1 = 无间隙计时模式（added task 09-08-continuous-timing） |
