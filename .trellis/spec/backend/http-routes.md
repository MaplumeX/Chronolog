...
| POST | `/api/timer/start` | yes | `{ categoryId, description?, tagIds? }`. Stamps `startedAt` (and the auto-stopped previous entry's `stoppedAt`) with the shared whole-second `nowIso` (`floorToSecondIso`, task 09-09-timer-timestamp-second-alignment) |
| POST | `/api/timer/stop` | yes | no running → 409. Reads the user's `continuous_timing` setting (task 09-08-continuous-timing): **off (default)** — stops the running entry, response `{ entry }` is the just-stopped entry; **on** — in one transaction sets the old entry's `stoppedAt = nowIso` and inserts a new running entry with the **same** `startedAt = nowIso`, `categoryId: null` (uncategorized), empty description, no tags; response `{ entry }` is the **new running entry** (`stoppedAt: null`). Clients distinguish "switched segment" from "fully stopped" solely by `entry.stoppedAt === null`. The single shared `nowIso` (floored to whole seconds — `floorToSecondIso`, task 09-09-timer-timestamp-second-alignment) guarantees zero gap (old `stoppedAt` === new `startedAt`); update-before-insert order satisfies the partial unique index; no retry loop needed (better-sqlite3 is synchronous — no interleaving inside the transaction). |
...
| POST | `/api/entries/:id/merge` | yes | merge with adjacent entry (task 09-08-merge-entries): body `{ direction: "prev" \| "next", keep: "self" \| "other" }` → `{ entry: EntryDto }`. Core logic `mergeEntries` (`server/src/entries.ts`), single transaction. Validation order: owned (404) → self stopped (409) → adjacent exists (409; prev = max `startedAt` among `startedAt <` self with `stoppedAt IS NOT NULL`, next = min `startedAt` among `startedAt >` self) → adjacent stopped (409) → adjacency re-check server-side (no third entry with `startedAt` strictly between the two — the server finds the neighbor itself, so stale frontend data can never skip an entry) → overlap backstop on the merged range excluding both ids (409 `OVERLAP`). Result: `startedAt = min` of the two, `stoppedAt = max`; keep entry's category/tags/description are kept as-is (whole-entry choice, no field migration), the other entry is deleted (`entry_tags` CASCADE). Running entries never merge, both directions. |
...
`POST /api/entries` and `PATCH /api/entries/:id` (`server/src/routes/entries.ts`) are the only endpoints that accept client-sent `startedAt` / `stoppedAt` (PATCH is a full update, stopped entries only; POST creates a stopped entry directly — the timeline drag-to-create flow, task 08-27-timeline-drag-create). Both reuse the same zod `updateBody` and shared validators (`checkTimeOrder` / `checkCategory` / `checkTags` / `checkOverlap`). `startedAt` / `stoppedAt` (and `/api/entries/boundary`'s `start` / `end`) use the shared `isoInstant` schema: `z.iso.datetime().transform(v => new Date(v).toISOString())` — zod only validates the format without normalizing it, and mixed formats (`...T12:00:00Z` vs `...T12:00:00.000Z`) break lexicographic comparisons (`Z` vs `.000Z`), so client-sent instants are normalized to the canonical `.000Z` millisecond form at the parse boundary. This keeps SQL TEXT `lt`/`gt` and `<=`/`>` string comparisons equal to instant comparisons (task 09-08-fix-overlap-string-compare); do not accept raw ISO strings into `time_entries` or window bounds without this normalization.
