...
/**
 * 老库迁移回归（task 09-08）：模拟「无 continuous_timing 列的库」，升级后默认 false、
 * 可通过 PATCH /api/profile 开启，二次打开幂等。
 */
describe("continuous timing migration", () => {
  let dir: string | undefined;
  afterEach(() => {
    if (dir && fs.existsSync(dir)) fs.rmSync(dir, { recursive: true, force: true });
    dir = undefined;
  ...
  it("upgrades a legacy db: adds users.continuous_timing default 0, toggleable via profile", async () => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "chronolog-ct-mig-"));
    const dbPath = path.join(dir, "legacy.db");
    const legacy = new Database(dbPath);
    legacy.pragma("journal_mode = WAL");
    legacy.exec(`
    ...
    legacy.prepare(
      `INSERT INTO users (id, username, password_hash, display_name, created_at)
       VALUES ('u1', 'legacy_ct_user', 'x', NULL, '2026-09-01T00:00:00.000Z')`,
    ).run();
    const sid = "ctMigrationSession00000000000000";
    legacy.prepare(
      `INSERT INTO sessions (id, user_id, expires_at, created_at)
       VALUES (?, 'u1', '2999-01-01T00:00:00.000Z', '2026-09-01T00:00:00.000Z')`,
    ).run(sid);
    legacy.close();

    // 新代码首次打开：migrate 补列，默认 false
    const app = await buildApp({
      dbPath,
      cookieSecure: false,
      sessionTtlSeconds: 604800,
    ...
    try {
      ...
      try {
        ...
        assert.ok(cols.includes("continuous_timing"), "users 应有 continuous_timing 列");
      } finally {
        sqlite.close();
      }
      ...
      assert.equal(me.statusCode, 200);
      assert.equal((json(me) as { continuousTiming: boolean }).continuousTiming, false);
      ...
      assert.equal(set.statusCode, 200);
      assert.equal((json(set) as { continuousTiming: boolean }).continuousTiming, true);
    } finally {
      await app.close();
    }
    ...
    try {
      ...
      assert.equal(me2.statusCode, 200);
      assert.equal((json(me2) as { continuousTiming: boolean }).continuousTiming, true);
    } finally {
      await app2.close();
    }
  });
