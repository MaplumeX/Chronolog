...
export function TimerBar(props: {
  ...
  return (
    <div className="min-w-0 flex-1 px-4 py-2">
      {/* 行 1：三列网格（3fr / auto / 2fr）——时长+按钮组固定在约 60% 线（居中偏右），
          不随输入框长度漂移；输入框限宽 max-w-xs（不占满左侧） */}
      <div className="grid grid-cols-[3fr_auto_2fr] items-center gap-3">
        <input
          className="min-w-0 max-w-xs bg-transparent text-lg tracking-tight outline-none placeholder:text-muted-foreground"
